...
def create_example_regions(
    env: FourRoomsEnv,
) -> tuple[UndirectedGraph[np.ndarray], ConnectedComponents[np.ndarray]]:
    ...
    # Reset the components graph to full connected components w/in state graph
    components.reset_edges(graph)
